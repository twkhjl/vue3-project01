...
relative">
      ...
      <button class="absolute right-4 hover:animate-pulse hover:font-extrabold hover:text-green-600"><i
          class="fa-solid fa-magnifying-glass"></i></button>
      <input class="w-[55vw] sm:w-auto h-[5vh] border-2 border-black rounded-full outline-none pl-4 my-2 text-xl 
  focus:outline-4 focus:outline-offset-0 focus:outline-gray-400" type="text" placeholder="輸入關鍵字...">
    </div>
      ...
      <tbody class="text-xl">
        ...
        <TransitionGroup name="fade">
          <template v-for="(item, idx) in data" :key="idx">
            <TableRow :category="item" :idx="idx + 1" :store="categoryStore" :data="data" @on-remove-event="removeItem">
            </TableRow>
          </template>
        </TransitionGroup>
