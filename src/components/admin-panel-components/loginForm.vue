...
<template>
  ...
  md:w-[35vw]">
        ...
        <div v-if="!isValid" class="text-xl text-red-400">{{ errMsg }}</div>

        <input v-model="name" @keyup="isValid = true" class="outline-none 
    border-2 border-gray-300 
    w-full h-[40px] rounded-md px-2 mb-4" type="text" placeholder="帳號">
        <input v-model="password" @keyup="isValid = true" class="outline-none 
    border-2 border-gray-300 
    w-full h-[40px] rounded-md px-2 mb-4" type="password" placeholder="密碼">
        ...
        <label class="relative flex items-center ml-0 mx-auto w-auto">
          <input type="checkbox" v-model="rememberMe" class="peer absolute left-0 h-0 w-0" />
          <span class="relative 
  block
  h-[35px] w-[35px]
